...
public class DefaultAliPayServiceImpl implements AliPayServiceAPI {
    ...
    public AliPayInfoVO getQRCode(String orderId) {
        String QRCodeAddress = tradePrecreate(orderId);
        if (StringUtils.isNotBlank(QRCodeAddress)){
            AliPayInfoVO aliPayInfoVO = new AliPayInfoVO();
            aliPayInfoVO.setOrderId(orderId);
            aliPayInfoVO.setQRCodeAddress(QRCodeAddress);
            return aliPayInfoVO;
        }
        return null;
    }
    ...
    public AliPayResultVO getOrderStatus(String orderId) {
        ...
        boolean flag = tradeQuery(orderId);
        if (flag) {
    ...
    private String tradePrecreate(String orderId) {
        ...
        String body = "购买电影片共花费"+orderVO.getOrderPrice();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "junyong.chen";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "junyong.chen";
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("/home/chan/graduation/qrcode/qr-%s.png",
                        response.getOutTradeNo());
                String fileName = String.format("qr-%s.png", response.getOutTradeNo());
                //log.info("filePath:" + filePath);
                File qrCodeImge = ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                // todo 网络传输考虑异步处理
                boolean isSuccess = ftpUtil.upload(fileName, qrCodeImge);
                if (!isSuccess) {
                    // 上传失败则重置二维码路径为空
                    filePath = "";
                    log.error("上传二维码失败，二维码文件名：",fileName);
                }
                break;
